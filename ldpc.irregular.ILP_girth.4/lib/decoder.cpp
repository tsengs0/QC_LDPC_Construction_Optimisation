...
LDPC_Decoder::LDPC_Decoder(
	...
	if(isLayered == true) {
		...
		base_matrix_connect = new uint32_t* [base_row_num];
		...
		for(uint32_t i=0; i<base_row_num; i++) {
			...
			base_matrix_connect[i] = new uint32_t[ dc_in[i] ];

			// For the irregular QC code structure
			uint32_t row_connect_cnt; row_connect_cnt=0;
			for(uint32_t j=0; j< base_col_num; j++) {
				if((base_matrix_connect_in[edge_cnt]-1) == j) {
					base_matrix.matrix[i][j]=1;
					base_matrix_connect[i][row_connect_cnt]=base_matrix_connect_in[edge_cnt];
					edge_cnt += 1;
					row_connect_cnt += 1;
				}
				else { // no connectivity, i.e., Z-by-Z zero matrix
		...
		for(uint32_t col=0; col<base_col_num; col++) {
			shift_patterns[col] = new uint16_t* [ dv_in[col] ];
			layeredMEMs[col].app_cache = new _LAYER_MEM_TYPE[lift_degree];
			layeredMEMs[col].c2v_cache = new _LAYER_MEM_TYPE* [ dv_in[col] ];
			for(uint32_t l=0; l<dv_in[col]; l++) {
...
void LDPC_Decoder::precompute_circulant_shift()
{
	...
	for(uint32_t col=0; col<base_col_num; col++) {
		uint32_t cnt; cnt=0;
		for(uint32_t l=0; l<base_row_num; l++) {
			if(base_matrix.circulantShift[l][col] != -1) {
				for(uint32_t submatrix_col=0; submatrix_col<lift_degree; submatrix_col++)
					shift_patterns[col][cnt][submatrix_col] = (uint16_t) ((submatrix_col+base_matrix.circulantShift[l][col]) % (int32_t)lift_degree);
				cnt += 1;
			}
		}
	}
...
void LDPC_Decoder::precompute_layer_sched()
{
	layer_indices_sched = new uint8_t* [layer_num];
	uint8_t *col_layer_cnt = new uint8_t[base_col_num]();
	for(uint8_t layer=0; layer<layer_num; layer++) {
		layer_indices_sched[layer] = new uint8_t[ row_degree_vec[layer] ];
		
		for(uint8_t col=0; col<row_degree_vec[layer]; col++) {
			layer_indices_sched[layer][col]=col_layer_cnt[ base_matrix_connect[layer][col]-1 ];
			col_layer_cnt[ base_matrix_connect[layer][col]-1 ] += 1;
			//std::cout << "Precompute layer sched: " << (uint32_t) layer_indices_sched[layer][col] 
			//	      << ", " << base_matrix_connect[layer][col]-1 << std::endl;
		}
	}
}
...
void LDPC_Decoder::layer_mem_reset()
{
	...
	for(col_id=0; col_id<base_col_num; col_id++)
		for(layer=0; layer<col_degree_vec[col_id]; layer++)
...
while(sim_results.errFrames < 1000) {
	...
	for(iter=0; iter<ITER_MAX; iter++) {
		uint8_t *col_layer = new uint8_t[base_col_num]();
		for(layer=0; layer<layer_num; layer++) {
			// Variable Node Units
			for(col_id=0; col_id<base_col_num; col_id++) {
				if(base_matrix.matrix[layer][col_id] != -1) {
					cnu_layer_cur = (uint8_t) ((col_layer[col_id]+0) % col_degree_vec[col_id]);
					for(submatrix_col=0; submatrix_col<lift_degree; submatrix_col++) {
			...
			for(col_id=0; col_id<row_degree_vec[layer]; col_id++) {
				absolute_col=base_matrix_connect[layer][col_id]-1;
				for(submatrix_col=0; submatrix_col<lift_degree; submatrix_col++) {
	/////////////////////////////////////////////////////////////////////
	...
	/////////////////////////////////////////////////////////////////////
					...
					channel_block[absolute_col].app_msg[ shift_patterns[absolute_col][ layer_indices_sched[layer][col_id] ][submatrix_col] ] += 
					layeredMEMs[absolute_col].c2v_cache[ layer_indices_sched[layer][col_id] ][ shift_patterns[absolute_col][ layer_indices_sched[layer][col_id] ][submatrix_col] ];
...
void LDPC_Decoder::layered_softProduct(uint8_t layer_id, uint16_t submatrix_col)
{
	...
	// to perform the product of tanh(.)
	_C2V_Msg temp;
	uint8_t absolute_col_i, absolute_col_j;
	uint8_t last_index = row_degree_vec[layer_id]-1;
	for(i=0; i<row_degree_vec[layer_id]; i++) {
		val_0 = 1.0;
		absolute_col_i=base_matrix_connect[layer_id][i]-1;
		for(j=0; j<row_degree_vec[layer_id]; j++) {
			absolute_col_j=base_matrix_connect[layer_id][j]-1;
			if(absolute_col_j== absolute_col_i) {
				if(i == last_index) break;
				else j += 1;
			}

			//std::cout << "shift_patterns: " << (uint32_t) shift_patterns[absolute_col_i][ layer_indices_sched[layer_id][i] ][submatrix_col]
			//	      << ", layer_indices_sched: " << (uint32_t) layer_indices_sched[layer_id][i] << std::endl;
			temp = channel_block[absolute_col_j].app_msg[ shift_patterns[absolute_col_i][ layer_indices_sched[layer_id][i] ][submatrix_col] ] / 2.0;
		...
		val_1 = 1.0-val_0;
		val_0 += 1.0;
		layeredMEMs[absolute_col_i].c2v_cache[ layer_indices_sched[layer_id][i] ][ shift_patterns[absolute_col_i][ layer_indices_sched[layer_id][i] ][submatrix_col] ] = log(val_0 / val_1);
